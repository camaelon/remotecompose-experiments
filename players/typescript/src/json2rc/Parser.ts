 ...
 */
import { Writer, Emit, TYPE_EXACT, TYPE_FILL, modBorder, modScroll, modClipRect, modVisibility, modClipRoundedRect, TYPE_WEIGHT, modWidthIn, modHeightIn, modWidth, modHeight, modPadding, modOffset, modBackground, modBackgroundId, modSemantics, modClick, SEMANTICS_MODE_SET, hostNamedAction,
    HOST_ACTION_STRING_TYPE, valueFloatExpressionChange, valueFloatChange,
    valueIntegerChange, valueStringChange, valueIntegerExpressionChange }
    from "./Writer";
import { floatToRawIntBits, idFromNanBits, asNanBits, f32FromBits } from "./WireBuffer";
import { HEADER_KEY_TO_TAG, Tag } from "./Header";
import { ExpressionParser } from "./Expr";
...
export class NotImplementedComponent extends Error {
    ...
    constructor(message: string) { super(message); this.name = "NotImplementedComponent"; }
}

/** Map of materialised variables, plus declarations waiting to be emitted at first use. */
export class DeferredVariables {
    private materialised = new Map<string, number>();
    private deferred = new Map<string, any>();
    resolver: ((name: string, cmd: any) => number) | null = null;

    defer(name: string, command: any): void { this.deferred.set(name, command); }
    ...
    isMaterialised(name: string): boolean { return this.materialised.has(name); }
    set(name: string, bits: number): void { this.materialised.set(name, bits); }
    remove(name: string): void { this.materialised.delete(name); this.deferred.delete(name); }
    ...
    get(name: string): number | undefined {
        ...
        if (pending === undefined || !this.resolver) return undefined;
...
function toInt32(v: number): number { return v | 0; }

export function parseColor(value: J): number {
    ...
    if (typeof value === "string") {
        ...
        if (s.length <= 7) v = (v | 0xff000000) >>> 0;
        return toInt32(v);
    }
...
const TEXT_ALIGN: Record<string, number> =
    { left: 1, "1": 1, right: 2, "2": 2, center: 3, "3": 3, justify: 4, "4": 4,
      start: 5, "5": 5 };
const OVERFLOW: Record<string, number> =
    { clip: 1, ellipsis: 3, visible: 2, start_ellipsis: 4, middle_ellipsis: 5 };

export function parseHAlign(a: string): number { return H_ALIGN[a.toLowerCase()] ?? 1; }
export function parseVAlign(a: string): number { return V_ALIGN[a.toLowerCase()] ?? 4; }
 ...
 */
export function hAlignOf(c: J, dflt: string): number {
    return parseHAlign(String(c.horizontalAlignment ?? c.horizontalArrangement ?? dflt));
}
export function vAlignOf(c: J, dflt: string): number {
    return parseVAlign(String(c.verticalAlignment ?? c.verticalArrangement ?? dflt));
}
export function parseTextAlign(a: string): number { return TEXT_ALIGN[a.toLowerCase()] ?? 5; }

/** `{"column": {...}}` is shorthand for `{"type": "column", ...}`. */
export function normalizeComponent(c: J): J {
    if (!("type" in c) && Object.keys(c).length === 1) {
        const key = Object.keys(c)[0];
        const val = c[key];
        const out: J = { type: key };
...
type PathItem = { bits: number } | number;
...
export function decodeBase64(input: string): Uint8Array {
    ...
    if (comma >= 0) str = str.slice(comma + 1);
    str = str.replace(/[\s]/g, "");
    ...
    for (let i = 0; i < end; i++) {
        ...
        if (v < 0) throw new NotImplementedComponent(`bad base64 character '${str[i]}'`);
        acc = (acc << 6) | v;
        bits += 6;
        if (bits >= 8) {
            bits -= 8;
 ...
 */
function arcToCubics(x0: number, y0: number, rx: number, ry: number, angle: number,
                     largeArc: boolean, sweep: boolean,
                     x1: number, y1: number): number[][] {
    if (rx === 0 || ry === 0) return [];          // caller emits a line instead
 ...
 */
export function parseSvgPath(pathData: string): PathItem[] {
    const out: PathItem[] = [];
    const cords = [0, 0, 0, 0, 0, 0];
    let cx = 0, cy = 0, sx = 0, sy = 0;
    const verb = (tag: number) => out.push({ bits: (tag | 0xff800000) >>> 0 });

    for (const seg of pathData.split(SVG_SPLIT)) {
        if (!seg || !/[A-Za-z]/.test(seg[0])) continue;
        const cmd = seg[0];
        const rest = seg.slice(1).trim();
        const values = rest ? rest.split(SVG_SEP).filter(Boolean).map(Number) : [];
        if (cmd === "M") {
            if (values.length < 2) throw new NotImplementedComponent(`SVG 'M' needs 2 values`);
            verb(PATH_MOVE); out.push(values[0], values[1]);
            cx = values[0]; cy = values[1]; sx = cx; sy = cy;
        } else if (cmd === "L") {
            for (let i = 0; i + 1 < values.length; i += 2) {
                verb(PATH_LINE); out.push(cx, cy, values[i], values[i + 1]);
                cx = values[i]; cy = values[i + 1];
            }
        } else if (cmd === "H") {
            for (const v of values) {
                verb(PATH_LINE); out.push(cx, cy, v, cords[1]);
                cx = v; cy = cords[1];
            }
        } else if (cmd === "V") {
            for (const v of values) {
                verb(PATH_LINE); out.push(cx, cy, cords[0], v);
                cx = cords[0]; cy = v;
            }
        } else if (cmd === "C") {
            ...
            for (let i = 0; i + 3 < values.length; i += 4) {
                verb(PATH_CUBIC);
                out.push(cx, cy, 2 * cords[0] - cords[2], 2 * cords[1] - cords[3],
            ...
            verb(PATH_CLOSE); cx = sx; cy = sy;
        } else {
            throw new NotImplementedComponent(
                `SVG path command '${cmd}' is not supported by the platform parser `
 ...
 * called "tex".
 */
function* eachResource(section: J): Generator<[string, J, J]> {
    const dictCfg = (v: J) =>
 ...
 */
function numBits(value: J): number {
    ...
    return floatToRawIntBits(n);
}
// ── 3D tables ────────────────────────────────────────────────────────────────
const PROJECTION: Record<string, number> = { perspective: 0, ortho: 1, orthographic: 1 };
const MATRIX_SUB: Record<string, number> =
    { identity: 0, translate: 1, scale: 2, rotate: 3, multiply: 4 };
const LIGHT_TYPE: Record<string, number> = { directional: 0, dir: 0, point: 1 };
const SURFACE: Record<string, number> = { general: 0, heightfield: 1, sphere: 2, cylinder: 3 };
const P3_CLEAR_DEPTH = 0, P3_MATERIAL = 1, P3_DEPTH_BIAS = 2;
/** Render modes are (backend << 1) | smoothBit. */
const MESH_MODE: Record<string, number> = {
...
const PRIMITIVE_PARAMS: Record<string, string[]> = {
...
const UV_MODE: Record<string, number> = { none: 0, uv: 1, uv2: 2, uv3: 3 };
const FLAG_SPLINE = 0x1, FLAG_TUBE_LEGACY = 0x2, FLAG_TUBE_PATH_DENSITY = 0x4,
      FLAG_TUBE_CAP = 0x8;
const TOUCH_STOP: Record<string, number> = {
 ...
 */
const VECTOR_FUNCTIONS: Record<string, number> = {
...
export class Parser {
     ...
     */
    variables = new DeferredVariables();
    colors = new Map<string, number>();
    paths = new Map<string, number>();
    bitmaps = new Map<string, number>();
    /** Supplied by the caller; see ConvertOptions.readFile. */
    private readFile?: (path: string) => Uint8Array;

    constructor(options: { readFile?: (path: string) => Uint8Array } = {}) {
    ...
    fbits(value: J): number {
        if (value === null || value === undefined) return NAN_BITS;
        if (typeof value === "boolean") return floatToRawIntBits(value ? 1 : 0);
        if (typeof value === "number") return floatToRawIntBits(value);
        if (typeof value === "string") {
            const special: Record<string, number> =
                { NaN: 0x7fc00000, Infinity: 0x7f800000, "-Infinity": 0xff800000 };
            if (value in special) return special[value] >>> 0;
            if (value === "max") return floatToRawIntBits(3.4028234663852886e38);
        ...
        if (value && typeof value === "object") return this.expr.parseExpression(value);
        throw new NotImplementedComponent(`float field type ${typeof value}`);
    }

    parse(doc: J): Uint8Array {
        ...
        if (apiLevel < 7) {
            ...
            throw new NotImplementedComponent(
                `apiLevel ${apiLevel}: only apiLevel >= 7 is implemented`);
        }
        ...
        for (const [key, value] of Object.entries(header)) {
            ...
            tags.push([tag, value as number | string]);
        }
        this.writer = new Writer(apiLevel, tags);
        this.expr = new ExpressionParser(this.writer, this.variables);
        ...
        if (doc.resources) { this.inFirstPass = true; this.parseResources(doc.resources);
                             this.inFirstPass = false; }
        const root = doc.root;
        if (root === undefined || root === null) return this.writer.encodeToByteArray();
        ...
        for (const item of items) {
            ...
            if (t === "resources" || t === "variable") continue;
            this.parseComponent(item);
        }
    ...
    private parseModifiers(raw: J): Emit[] {
        const out: Emit[] = [];
        for (const mod of raw ?? []) {
            ...
            const entries: [string, J][] =
                typeof mod === "string" ? [[mod, "NaN"]] : Object.entries(mod);
            for (const [key, value] of entries) {
                const k = key.toLowerCase();
                if (k === "fillmaxsize") {
                    const v = this.fbits(value);
                    out.push(modWidth(TYPE_FILL, v), modHeight(TYPE_FILL, v));
                } else if (k === "fillmaxwidth") {
                    out.push(modWidth(TYPE_FILL, this.fbits(value)));
                } else if (k === "fillmaxheight") {
                    out.push(modHeight(TYPE_FILL, this.fbits(value)));
                } else if (k === "width") {
                    out.push(modWidth(TYPE_EXACT, this.fbits(value)));
                } else if (k === "height") {
                    out.push(modHeight(TYPE_EXACT, this.fbits(value)));
                    ...
                    out.push(value.startsWith("$") || value.startsWith("@")
                        ? modBackgroundId(this.resolveColor(value))
                        : modBackground(parseColor(value)));
                } else if (k === "border") {
                    const b = (value ?? {}) as Record<string, J>;
                    out.push(modBorder(Number(b.width), Number(b.cornerRadius),
                                       this.resolveColor(b.color), Math.trunc(Number(b.shape ?? 0))));
                    ...
                    if (Array.isArray(value)) {
                        out.push(modWidth(TYPE_EXACT, numBits(value[0])),
                                 modHeight(TYPE_EXACT, numBits(value[1])));
                    } else {
                        const v = this.fbits(value);
                        out.push(modWidth(TYPE_EXACT, v), modHeight(TYPE_EXACT, v));
                    }
                } else if (k === "weight" || k === "horizontalweight") {
                    out.push(modWidth(TYPE_WEIGHT, numBits(value)));
                } else if (k === "verticalweight") {
                    out.push(modHeight(TYPE_WEIGHT, numBits(value)));
                } else if (k === "widthin") {
                    const wi = value as J[];
                    out.push(modWidthIn(this.fbits(wi[0]), this.fbits(wi[1])));
                } else if (k === "heightin") {
                    const hi = value as J[];
                    out.push(modHeightIn(this.fbits(hi[0]), this.fbits(hi[1])));
                } else if (k === "onclick" || k === "multiclick") {
                    let raw = value, clickType = 0;
                    if (k === "multiclick") {
                        const mv = (value ?? {}) as Record<string, J>;
                        clickType = Number(mv.clickType ?? 0);
                        raw = mv.actions as J;
                    }
                    out.push(modClick(this.parseActions(raw), clickType));
                } else if (k === "semantics") {
                    const sem = (value ?? {}) as Record<string, J>;
                    const tid = (field: string) =>
                        field in sem ? this.writer.addText(String(sem[field])) : 0;
                    out.push(modSemantics(
                        tid("contentDescription"),
                        -1,                       // role: there is no JSON key for it upstream either
                        tid("text"),
                        tid("stateDescription"),
                        SEMANTICS_MODE_SET,
                        sem.enabled === undefined ? true : Boolean(sem.enabled),
                        Boolean(sem.clickable ?? false)));
                } else if (k === "offset") {
                    if (Array.isArray(value)) {
                        out.push(modOffset(this.fbits(value[0]), this.fbits(value[1])));
                    } else if (value && typeof value === "object") {
                        const o = value as Record<string, J>;
                        out.push(modOffset(this.fbits(o.x ?? 0), this.fbits(o.y ?? 0)));
                    } else {
                        throw new NotImplementedComponent(
                            `offset modifier value ${JSON.stringify(value)}`);
                    }
                } else if (k === "padding") {
                    const b = this.paddingBits(value);
                    out.push(modPadding(b[0], b[1], b[2], b[3]));
                } else {
     ...
     */
    private paddingBits(value: J): [number, number, number, number] {
        if (Array.isArray(value)) {
            if (value.length < 4) {
                throw new NotImplementedComponent(
                    `padding list needs 4 values, got ${value.length}`);
            }
            return [numBits(value[0]), numBits(value[1]), numBits(value[2]), numBits(value[3])];
        }
        if (value && typeof value === "object") {
            const g = (k: string) => numBits(value[k] ?? 0);
    ...
    private parseComponent(component: J): void {
        ...
        if (ctype === "column" || ctype === "row" || ctype === "box") {
            if (ctype === "box" && kids.length === 0) {
                ...
                this.writer.boxLeaf(
                    hAlignOf(component, "center"),
                    vAlignOf(component, "center"), mods);
                return;
            }
            const h = hAlignOf(component, "start");
            const v = vAlignOf(component, "top");
            if (ctype === "column") this.writer.startColumn(h, v, mods);
            else if (ctype === "row") this.writer.startRow(h, v, mods);
            else this.writer.startBox(h, v, mods);
            for (const k of kids) this.parseComponent(k);
            this.writer.endContainer();
            return;
        }
        if (ctype === "fitbox") {
            this.writer.startFitBox(
                hAlignOf(component, "center"),
                vAlignOf(component, "center"), mods);
        ...
        if (ctype === "spacer") {
            ...
            const m = mods.length ? mods : [modWidth(TYPE_WEIGHT, floatToRawIntBits(1.0))];
            this.writer.startBox(0, 0, m);
            this.writer.endContainer();
            return;
        }
        if (ctype === "flow") {
            this.writer.startFlow(
                hAlignOf(component, "start"),
                vAlignOf(component, "top"), mods,
                Math.trunc(Number(component.maxColumns ?? 2147483647)));
            for (const k of (component.children ?? []).map(normalizeComponent)) {
                this.parseComponent(k);
            }
            this.writer.endFlow();
        ...
        if (ctype === "global") {
            ...
            if (this.inFirstPass) {
                ...
                for (const k of kidsAll) {
                    if (FIRST_PASS_TYPES.has(String(k.type).toLowerCase())) this.parseComponent(k);
                }
        ...
        if (ctype === "text") { this.parseText(component, mods); return; }
        if (ctype === "canvas") {
    ...
    private normalizeCommand(c: J): J {
        if (!("type" in c) && Object.keys(c).length === 1) {
            const key = Object.keys(c)[0];
            const val = c[key];
            const out: J = { type: key };
            ...
            else if (["drawpath", "pathappendclose"].includes(key.toLowerCase())) out.path = val;
            else out.value = val;
            return out;
        }
        return c;
    ...
    private paintSetter(key: string, src: J, ints: number[]): void {
        if (key === "shader") {
            ...
            if (typeof v !== "number") {
                throw new NotImplementedComponent(
                    "paint shader: only a numeric shader id is supported; an AGSL runtime "
            ...
            if (v === undefined) throw new NotImplementedComponent(`paint style ${src.style}`);
            ints.push(PB.STYLE | (v << 16));
        } else if (key === "alpha") ints.push(PB.ALPHA, this.fbits(src.alpha));
        else if (key === "width" || key === "strokeWidth") {
            // strokeWidth is the preferred spelling and wins when both are present.
            ints.push(PB.STROKE_WIDTH,
                      this.fbits("strokeWidth" in src ? src.strokeWidth : src.width));
        ...
        } else if (key === "textSize") ints.push(PB.TEXT_SIZE, this.fbits(src.textSize));
        else if (key === "linearGradient") this.gradient(src.linearGradient, ints, false);
        else if (key === "sweepGradient") this.gradient(src.sweepGradient, ints, true);
        else throw new NotImplementedComponent(`paint key '${key}'`);
    }
     ...
     */
    private gradient(g: J, ints: number[], sweep: boolean): void {
        const colorsArr = (g.colors ?? []) as J[];
        const colors: number[] = [];
        let idMask = 0;
        ...
        const stops = g.stops as J[] | undefined;
        ...
        if (stops) for (const st of stops) ints.push(floatToRawIntBits(Number(st)));
     ...
     */
    private declareBitmap(name: string, value: J): number {
        let spec: Record<string, J> = typeof value === "string"
            ? { file: value } : (value as Record<string, J>);
        ...
        if ("base64" in spec) {
            try {
                ...
                throw new NotImplementedComponent(
                    `bitmap '${name}': bad base64: ${(e as Error).message}`);
            }
        } else if ("file" in spec) {
            if (!this.readFile) {
                throw new NotImplementedComponent(
                    `bitmap '${name}' names a file, but this converter was given no file `
            ...
            try {
                ...
                throw new NotImplementedComponent(
                    `bitmap '${name}': ${(e as Error).message}`);
            }
        } else {
     ...
     */
    private resolveTextId(obj: J): number {
        ...
        if (typeof obj === "number") {
            ...
            return idFromNanBits(this.writer.addFloatConstant(obj));
        }
        if (obj && typeof obj === "object") {
            const kind = String((obj as J).type ?? "").toLowerCase();
            if (kind === "textfromfloat") {
                const vo = obj as J;
                return this.writer.addTextFromFloat(
                    this.fbits(vo.value), Number(vo.whole ?? 0),
                    Number(vo.decimal ?? 3), Number(vo.flags ?? 0));
            }
            throw new NotImplementedComponent(
                kind === "textmerge"
        ...
        if (text.includes("@") && /[+\-*/%]/.test(text)) {
            throw new NotImplementedComponent(
                `text id '${text}' is an integer expression, which needs integer-variable `
        ...
        if (/^[@$]/.test(text)) {
            ...
            if (bits !== undefined) {
                ...
                return Math.trunc(f32FromBits(bits));       // a plain float holding an id
            }
        }
        if (text.startsWith("#")) return this.writer.addColor(toInt32(parseColor(text)));
        return this.writer.addText(text);
    }

    /** A bitmap reference: a numeric id, or `@name`/`$name` from `resources.bitmaps`. */
    private bitmapId(value: J): number {
        if (typeof value === "string") {
            ...
            throw new NotImplementedComponent(
                `bitmap '${value}' not found (declared: ${known})`);
        }
        return Math.trunc(Number(value));
    }

    /** `mode` plus the wireframe flags, resolved into the packed mode word. */
    private meshMode(command: J): number {
        const raw = command.mode ?? "software-flat";
        let mode: number;
        if (typeof raw === "number") mode = Math.trunc(raw);
        else {
        ...
        if (command.wireframe) {
            mode |= MODE_WIREFRAME;
            for (const e of (command.edges ?? []) as J[]) {
    ...
    private points(command: J, key: string, dim: number): number[] {
        const pts = command[key] as J[] | undefined;
        if (!pts || !pts.length) {
        ...
        for (const pt of pts) {
            if (!Array.isArray(pt) || pt.length !== dim) {
                throw new NotImplementedComponent(
                    `meshPrimitive3D: ${key} entries need ${dim} components, `
                    + `got ${Array.isArray(pt) ? pt.length : 0}`);
            }
            out.push(...(pt as J[]).map(Number));
        }
        return out;
    }
     ...
     */
    private resolveId(value: J): number {
        if (typeof value === "boolean") {
            throw new NotImplementedComponent(`expected an id, got boolean ${value}`);
        }
        if (typeof value === "number") {
            if (Number.isInteger(value)) return value;
            return idFromNanBits(this.writer.addFloatConstant(value));
        }
        return idFromNanBits(this.fbits(value));
    }
     ...
     */
    private parseActions(raw: J): Emit[] {
        ...
        for (const item of items) {
            if (!item || typeof item !== "object" || Array.isArray(item)) {
                throw new NotImplementedComponent(
                    `action ${JSON.stringify(item)} (expected an object)`);
            }
            const a = item as Record<string, J>;
            ...
            if (t === "hostnamedaction" || t === "hostmetadataaction") {
                ...
                const type = value !== null
                    ? Number(a.actionType ?? HOST_ACTION_STRING_TYPE) : -1;
                actions.push((w) => {
                    const textId = w.addText(name);
                    const valueId = value !== null ? w.addText(value) : -1;
                    hostNamedAction(textId, type, valueId)(w);
                });
                ...
                actions.push(valueFloatExpressionChange(
                    targetId, idFromNanBits(this.fbits(rawValue))));
            } else if (t === "valuefloatchange") {
                if (target === undefined || target === null) {
                    throw new NotImplementedComponent('valueFloatChange needs a "target"');
                }
                actions.push(valueFloatChange(this.resolveId(target), this.fbits(a.value)));
            } else if (t === "valueintegerchange") {
                if (target === undefined || target === null) {
                    throw new NotImplementedComponent('valueIntegerChange needs a "target"');
                }
                actions.push(valueIntegerChange(this.resolveId(target), Number(a.value)));
                ...
                if (typeof value === "string"
                    ...
                    actions.push((w) => valueStringChange(targetId, w.addText(value))(w));
                } else {
                    actions.push(valueStringChange(targetId, this.resolveId(value)));
                }
                ...
                actions.push(valueIntegerExpressionChange(Math.trunc(target), Math.trunc(value)));
            } else {
     ...
     */
    private resolvePath(value: J): number {
        ...
        if (s.startsWith("$paths.") || s.startsWith("@paths.")) {
            ...
            if (id === undefined) throw new NotImplementedComponent(`Path not found: ${name}`);
            return id;
        }
    ...
    private resolveColor(value: J): number {
        if (typeof value === "string" &&
            (value.startsWith("$colors.") || value.startsWith("@colors."))) {
            const name = value.slice(8);
            const id = this.colors.get(name);
            if (id === undefined) throw new NotImplementedComponent(`Color not found: ${name}`);
            return id;
        }
        return toInt32(parseColor(value));
    }

    private buildPaintBundle(command: J): number[] {
        const ints: number[] = [];
        if ("ops" in command) {
        ...
        for (const k of PAINT_KEYS_IGNORED) {
            if (k in command) {
                throw new NotImplementedComponent(
                    `paint key '${k}' is ignored as a direct key by the reference parser — ` +
    ...
    private parseCommand(raw: J): void {
        ...
        if (t === "variable") { this.parseVariable(command); return; }
        if (t === "paint") { w.paintValues(this.buildPaintBundle(command)); return; }
        if (t === "setcolor") { w.paintValues([PB.COLOR, toInt32(parseColor(command.color))]); return; }
        if (t === "setstyle") {
            const v = STYLE[String(command.style).toLowerCase()];
            if (v === undefined) throw new NotImplementedComponent(`setStyle ${command.style}`);
            w.paintValues([PB.STYLE | (v << 16)]); return;
        }
        if (t === "setstrokewidth") { w.paintValues([PB.STROKE_WIDTH, f("width")]); return; }
        if (t === "drawrect") { w.drawRect(f("left"), f("top"), f("right"), f("bottom")); return; }
        if (t === "drawoval") { w.drawOval(f("left"), f("top"), f("right"), f("bottom")); return; }
        if (t === "drawcircle") { w.drawCircle(f("cx"), f("cy"), f("radius")); return; }
        if (t === "drawline") { w.drawLine(f("x1"), f("y1"), f("x2"), f("y2")); return; }
        if (t === "drawroundrect") {
            w.drawRoundRect(f("left"), f("top"), f("right"), f("bottom"), f("rx"), f("ry")); return;
        }
        if (t === "drawarc") {
        ...
        if (t === "drawtextanchored") {
            w.drawTextAnchored(this.resolveTextId(command.text), f("x"), f("y"),
                               f("panX"), f("panY"), Number(command.flags ?? 0));
            return;
        }
        if (t === "save") {
        ...
        if (t === "restore") { w.matrixRestore(); return; }
        if (t === "translate") { w.matrixTranslate(f("dx"), f("dy")); return; }
        if (t === "scale") { w.matrixScale(f("sx"), f("sy")); return; }
        if (t === "rotate") {
            ...
            w.matrixRotate(f("angle"), px, py); return;
        }
        if (t === "drawpath") { w.drawPath(this.resolvePath(command.path)); return; }
        if (t === "pathcreate") {
        ...
        if (t === "pathappendlineto") {
            w.pathAppendLineTo(this.resolvePath(command.path), f("x"), f("y")); return;
        }
        if (t === "pathappendclose") { w.pathAppendClose(this.resolvePath(command.path)); return; }
        if (t === "createparticles") {
            ...
            const names = (command.variables as J[]).map(String);
            const inits = (command.initialValues as J[]).map((v) => this.expr.infixToRpn(String(v)));
            if (inits.length !== names.length) {
                throw new NotImplementedComponent(
                    `createParticles: ${names.length} variables but ${inits.length} initialValues`);
            }
            const [systemId, varIds] = w.createParticles(inits, Math.trunc(Number(command.count)));
            this.variables.set(String(command.id), asNanBits(systemId));
            names.forEach((n, i) => this.variables.set(n, asNanBits(varIds[i])));
            return;
        }
        if (t === "particlesloop") {
            ...
            const eqs = (command.equations as J[]).map((e) => this.expr.infixToRpn(String(e)));
        ...
        if (t === "particlescomparison") {
            const eqs = (key: string): number[][] | null => {
                const raw = command[key] as J[] | undefined;
            ...
            w.particlesCompareStart(f("systemId") & 0x007fffff,
                                    Math.trunc(Number(command.flags ?? 0)),
                                    f("min"), f("max"), cond, eqs("then1"), then2);
            for (const c of command.commands ?? []) this.parseCommand(c);
            w.containerEnd();
            return;
        }
        ...
        if (t === "cleardepth3d") { w.paint3DState(P3_CLEAR_DEPTH, []); return; }
        if (t === "material3d") {
            w.paint3DState(P3_MATERIAL, [this.fbits(command.specular ?? 0.0),
                                         this.fbits(command.shininess ?? 32.0)]); return;
        }
        if (t === "depthbias3d") {
            w.paint3DState(P3_DEPTH_BIAS, [this.fbits(command.constant ?? 0.0),
                                           this.fbits(command.slope ?? 0.0)]); return;
        }
        if (t === "drawmesh3d") { w.drawMesh3D(Number(command.mesh), this.meshMode(command)); return; }
        if (t === "texture3d") {
        ...
        if (t === "camera3d") {
            ...
            const projParams = proj === 0
                ...
                : [f("left"), f("right"), f("bottom"), f("top"),
                   this.fbits(command.near ?? 0.1), this.fbits(command.far ?? 100.0)];
            const eye = (command.eye ?? [0, 0, 4]) as J[];
            const center = (command.center ?? [0, 0, 0]) as J[];
            const up = (command.up ?? [0, 1, 0]) as J[];
            const view = [...eye, ...center, ...up].map((v) => this.fbits(v));
            if (view.length !== 9) {
                throw new NotImplementedComponent(
                    "camera3D needs eye/center/up of 3 components each");
            }
            w.setCamera3D(proj, projParams, view); return;
        }
        if (t === "matrix3d") {
            ...
            if (!("op" in command)) {
                ...
                if (extra.length) {
                    throw new NotImplementedComponent(
                        'matrix3D needs an "op" (identity/translate/scale/rotate/multiply); '
            ...
            if (sub === 0) args = [];
                ...
                const axis = (command.axis ?? [0, 1, 0]) as J[];
                args = [f("angle"), ...axis.map((v) => this.fbits(v))];
                if (args.length !== 4) {
                    throw new NotImplementedComponent("matrix3D rotate axis needs 3 components");
                }
            } else {
                const m = command.m as J[];
                if (!m || m.length !== 16) {
                    throw new NotImplementedComponent(
                        `matrix3D multiply needs 16 floats, got ${m ? m.length : 0}`);
                }
                args = m.map((v) => this.fbits(v));
            }
            w.matrix3DOp(sub, args); return;
        }
        if (t === "lights3d") {
            const types: number[] = [], colors: number[] = [], params: number[] = [];
            for (const light of (command.lights ?? []) as J[]) {
                ...
                const vec = (ltype === 1 ? light.pos : light.dir) as J[] | undefined;
                if (!vec || vec.length !== 3) {
                    throw new NotImplementedComponent(
                        `lights3D ${tname} needs a 3-component ${ltype === 1 ? "pos" : "dir"}`);
                }
                params.push(...vec.map((v) => this.fbits(v)),
                            this.fbits(light.intensity ?? 1.0));
            }
            w.setLights3D(types, colors, params); return;
        }
        if (t === "definemesh3d") {
            const verts = (command.verts as J[]).map((v) => this.fbits(v));
            const normalsRaw = (command.normals ?? []) as J[];
            const uvRaw = (command.uv ?? []) as J[];
            const normals = normalsRaw.length ? normalsRaw.map((v) => this.fbits(v)) : null;
            const uv = uvRaw.length ? uvRaw.map((v) => this.fbits(v)) : null;
            const idx = (command.indices as J[]).map((i) => Math.trunc(Number(i)));
            if (idx.length % 3) {
                throw new NotImplementedComponent(
                    `defineMesh3D indices length ${idx.length} is not a multiple of 3`);
            }
            if (verts.length % 3) {
                throw new NotImplementedComponent(
                    `defineMesh3D verts length ${verts.length} is not a multiple of 3`);
            }
            if (normals && normals.length !== verts.length) {
                throw new NotImplementedComponent(
                    `defineMesh3D normals ${normals.length} != verts ${verts.length}`);
            }
            if (uv && uv.length !== (verts.length / 3) * 2) {
                throw new NotImplementedComponent(
                    `defineMesh3D uv ${uv.length} != 2 per vertex (${(verts.length / 3) * 2})`);
            }
            w.defineMesh3D(Number(command.id), idx, verts, normals, uv); return;
        }
        if (t === "vectorexpression") {
            const dim = Math.trunc(Number(command.dimension ?? 3));
            if (![2, 3, 4].includes(dim)) {
                throw new NotImplementedComponent(
                    `vectorExpression dimension ${dim} (expected 2, 3 or 4)`);
            }
            ...
            if (command.name) {
                ...
                for (let k = 0; k < dim; k++) {
                    this.variables.set(`${command.name}.${VECTOR_COMPONENTS[k]}`,
                                       asNanBits(base + k));
                }
            }
            return;
        ...
        if (t === "touchexpression") {
            ...
            const bits = w.touchExpression(
                this.fbits(command.defaultValue ?? 0.0), minB, this.fbits(command.max ?? 1.0),
                Math.trunc(Number(mode)), velB, Math.trunc(Number(command.touchEffects ?? 0)),
                ops,
                command.touchSpec ? (command.touchSpec as J[]).map(Number) : null,
                command.easingSpec ? (command.easingSpec as J[]).map(Number) : null);
            if (command.name) this.variables.set(String(command.name), bits);
            return;
        }
        if (t === "meshprimitive3d") {
            ...
            if (!(kind in PRIMITIVE)) {
                throw new NotImplementedComponent(
                    `meshPrimitive3D primitive '${command.primitive}' `
                    + `(expected one of ${Object.keys(PRIMITIVE).sort().join(", ")})`);
            }
            const ptype = PRIMITIVE[kind];
            let flagsExtra = 0;
            const extraChannels: number[][] = [];
            let scalars: number[];
            const fbAll = (xs: J[]) => xs.map((v) => this.fbits(v));
            if ("params" in command) {
                scalars = fbAll(command.params as J[]);
                ...
                if (command.legacy) flagsExtra |= FLAG_TUBE_LEGACY;
                if (command.spline) flagsExtra |= FLAG_SPLINE;
            } else if (kind === "profiletube") {
                // [nPoints, (ringsPerSpan), x,y,z * n, r0..rk]
                const pts = this.points(command, "points", 3);
                const radii = command.radii as J[] | undefined;
                ...
                if (command.capped === undefined || command.capped) flagsExtra |= FLAG_TUBE_CAP;
            } else if (kind === "sweep") {
                ...
                const centre = (command.center ?? [0, 0, 0]) as J[];
                if (centre.length !== 3) {
                    throw new NotImplementedComponent("meshPrimitive3D sweep: center needs 3 values");
                }
                const bool = (v: J, d: boolean) => this.fbits((v === undefined ? d : v) ? 1 : 0);
                ...
                if ("scales" in command) extraChannels.push(fbAll(command.scales as J[]));
                if ("twists" in command) {
                    ...
                    extraChannels.push(fbAll(command.twists as J[]));
                }
                ...
                const contours = command.contours as J[] | undefined;
                ...
                for (const ring of contours as J[][]) {
                    scalars.push(this.fbits(ring.length));
                    for (const pt of ring as J[][]) {
                ...
                for (const key of PRIMITIVE_PARAMS[kind]) {
                    if (key === "center" || key === "from" || key === "to") {
                        const vec = (command[key] ?? [0, 0, 0]) as J[];
                        if (vec.length !== 3) {
                            throw new NotImplementedComponent(
                                `meshPrimitive3D ${kind}: ${key} needs 3 components`);
                        }
                        scalars.push(...fbAll(vec));
                    } else {
                        if (!(key in command)) {
                            throw new NotImplementedComponent(
                                `meshPrimitive3D ${kind}: missing '${key}' `
                ...
                throw new NotImplementedComponent(
                    `meshPrimitive3D ${kind}: pass an explicit "params" list`);
            }
            ...
            const channels: number[][] = [scalars, ...extraChannels];
            for (const extra of (command.channels ?? []) as J[][]) channels.push(fbAll(extra));
            if (kind === "lathe" && "profile" in command) {
                const prof: number[] = [];
                for (const pt of command.profile as J[][]) {
                    if (!Array.isArray(pt) || pt.length !== 2) {
                        throw new NotImplementedComponent(
            ...
            w.meshPrimitive3D(Number(command.id), ptype,
                              this.fbits(command.segments ?? 0), flags, channels);
            return;
        }
        if (t === "meshexpression3d") {
            ...
            if (!(surface in SURFACE)) {
                throw new NotImplementedComponent(
                    `meshExpression3D surface '${command.surface}' `
            ...
            const rpn = (expr: J): number[] => {
                ...
                try {
                    ...
                    for (const k of ["u", "v"]) {
                        if (saved[k] === undefined) this.variables.remove(k);
                        else this.variables.set(k, saved[k]!);
                    }
                }
            };
            const need = (key: string): J => {
                if (!(key in command)) {
                    throw new NotImplementedComponent(
                        `meshExpression3D ${surface}: missing '${key}'`);
                }
                return command[key];
            };
            const rng = (key: string, dflt?: number[]): number[] => {
                const r = (command[key] ?? dflt) as J[] | undefined;
                if (!r || r.length !== 2) {
                    throw new NotImplementedComponent(
                        `meshExpression3D ${surface}: '${key}' must be [min, max]`);
                }
                return [this.fbits(r[0]), this.fbits(r[1])];
            };
            let params: number[], pos: number[][];
            ...
            const normal = ((command.normal ?? []) as J[]).map(rpn);
        ...
        if (t === "loop") {
            ...
            const stepBits = "step" in command ? f("step") : floatToRawIntBits(1.0);
            ...
            for (const c of command.commands ?? []) this.parseCommand(c);
            if (had && prev !== undefined) this.variables.set(index, prev);
            else this.variables.remove(index);
            w.endLoop();
            return;
        }
        if (t === "cliprect") { w.clipRect(f("left"), f("top"), f("right"), f("bottom")); return; }
        throw new NotImplementedComponent(`canvas command '${t}'`);
    }

    private parseVariable(command: J): void {
        const name = String(command.name);
        const vtype = String(command.vtype ?? "float");
        if (vtype !== "float") {
            throw new NotImplementedComponent(`variable vtype '${vtype}'`);
        }
        if (command.export) throw new NotImplementedComponent("named/export float variable TBD");
    ...
    private materialiseFloatVar(command: J): number {
        const value = command.value;
        if (typeof value === "number") return this.writer.addFloatConstant(value);
        if (value && typeof value === "object" && !Array.isArray(value)) {
            const kind = String((value as J).type ?? "").toLowerCase();
            if (kind === "textfromfloat" || kind === "textmerge") {
                ...
                return floatToRawIntBits(this.resolveTextId(value));
            }
        }
     ...
     */
    private static RESOURCE_ORDER =
        ["v_dims", "colors", "paths", "floatArrays", "variables",
         "integers", "matrices", "bitmaps", "sounds"];

    private parseResources(section: J): void {
        const order: string[] = (section.order as string[] | undefined)
        ...
        for (const key of order) {
            ...
            if (!["colors", "floatArrays", "variables", "bitmaps"].includes(key)) {
                throw new NotImplementedComponent(
                    `resource type '${key}' (colors/floatArrays/variables/bitmaps only)`);
            }
            this.parseResourceSection(key, section[key]);
        }
    }

    private parseResourceSection(key: string, sectionValue: J): void {
        const section: J = { [key]: sectionValue };
        if (key === "bitmaps") {
        ...
        if ("floatArrays" in section) {
            for (const [name, cfg, value] of eachResource(section.floatArrays)) {
                const raw = value && typeof value === "object" && "value" in value
                    ? value.value : value;
                const data = (raw as any[]).map(Number);
        ...
        if ("variables" in section) {
            for (const [name, cfg, value] of eachResource(section.variables)) {
                ...
                if (val && typeof val === "object"
                    ...
                    if (named) this.writer.setFloatName(bits & 0x007fffff, name);
                }
            }
        }
    }

    private parseText(component: J, mods: Emit[]): void {
        if (!this.profiles) {
            ...
            throw new NotImplementedComponent(
                "text without a `profiles` header tag — add \"profiles\": 513");
        }
        ...
        if (value !== undefined && value !== null) {
            ...
            textId = this.writer.addTextFromFloat(this.fbits(tff.value),
                Number(tff.whole ?? 0), Number(tff.decimal ?? 0), Number(tff.flags ?? 0));
        } else {
        ...
        this.writer.textComponent(textId, {
            ...
            fontSizeBits: component.fontSize !== undefined
                ? this.fbits(component.fontSize) : floatToRawIntBits(36.0),
            fontWeightBits: component.fontWeight !== undefined
                ? this.fbits(component.fontWeight) : floatToRawIntBits(400.0),
 ...
 */
export function unwrap(doc: J): J {
    if (!("root" in doc) && doc.json && typeof doc.json === "object") return doc.json;
    return doc;
}

/** Anything that can hand the converter the bytes of a path named in the JSON. */
export type FileReader = (path: string) => Uint8Array;

export interface ConvertOptions {
    /**
     * Resolves a `file:` reference in `resources.bitmaps`. The core has no filesystem
     * dependency on purpose — it runs in a browser — so the caller supplies this. Paths are
     * passed through verbatim; resolving them relative to the document is the caller's job,
     * as it is the caller who knows where the document came from.
     */
    readFile?: FileReader;
}

export function convert(json: string | J, options: ConvertOptions = {}): Uint8Array {
