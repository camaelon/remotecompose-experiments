...
exports.ExpressionParser = exports.ExpressionError = void 0;
exports.isVariableRef = isVariableRef;
exports.variableNameFromRef = variableNameFromRef;
/**
...
class ExpressionParser {
    ...
    isVariable(token) {
        if (COMPONENT_WIDTH_VARS.has(token) || COMPONENT_HEIGHT_VARS.has(token))
            return true;
    ...
    variableNanBits(token) {
        if (COMPONENT_WIDTH_VARS.has(token))
            return this.writer.addComponentValue(0);
        if (COMPONENT_HEIGHT_VARS.has(token))
            return this.writer.addComponentValue(1);
        if (token in SYSTEM_VARS)
            return (0, WireBuffer_1.asNanBits)(SYSTEM_VARS[token]);
        const name = isVariableRef(token) ? variableNameFromRef(token) : token;
    ...
    infixToRpn(expression, extraFunctions) {
        const FUNCS = extraFunctions ? { ...FUNCTIONS, ...extraFunctions } : FUNCTIONS;
        const output = [];
        const stack = [];
        let lastWasOperator = true;
        const emitOp = (op) => {
            if (op in OPERATORS)
                output.push((0, WireBuffer_1.asNanBits)(OFFSET + OPERATORS[op]));
            else if (op in FUNCS && FUNCS[op] !== null) {
                output.push((0, WireBuffer_1.asNanBits)(OFFSET + FUNCS[op]));
            }
        };
        for (const token of this.tokenize(expression)) {
            if (this.isNumber(token)) {
                output.push((0, WireBuffer_1.floatToRawIntBits)(parseFloat(token)));
                ...
                if (stack.length >= 2 && stack[stack.length - 1] === "(" &&
                    ...
                    output.push((0, WireBuffer_1.asNanBits)(OFFSET + RAND_SEED_OP));
                ...
                if (stack.length && stack[stack.length - 1] in FUNCS) {
                    ...
                    if (fn === "seed") {
                        output.push((0, WireBuffer_1.asNanBits)(OFFSET + RAND_SEED_OP));
                        output.push((0, WireBuffer_1.floatToRawIntBits)(1.0));
                    }
                    else if (fn !== "seed_2arg")
                        emitOp(fn);
                }
                lastWasOperator = false;
            }
